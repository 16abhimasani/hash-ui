...
import { SEASON_TO_ART_FACTORY_SERVER } from '@hash/seasons/server';
...
export const getMetadata = async (season: Season, hash: string) => {
  ...
  const artFactoryServer = SEASON_TO_ART_FACTORY_SERVER[season];
  ...
  return {
    ...
    image: !!artFactoryServer?.getImage
      ? await artFactoryServer.getImage(gene)
      : getArtworkPreviewUrl(paddedHashStr),
